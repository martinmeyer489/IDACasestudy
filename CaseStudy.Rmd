...
The datasets K1BE1, K1BE2, and K1DI2 contain the columns "Produktionsdatum_Origin_01011970" and "origin" but they seem to be mixed up. In the following step this is corrected (*Produktionsdatum has still some mistakes in it (i.e. 0001 instead of 2001) but we dont need it anyway I think*)
...
Afterwards,the two variables of vehcile IDs can be united and the NA-values can be removed, because no row contains information in both ID columns.
...
The IDs formats are changed into factors to check whether each part is assigned to one component and each component to one vehicle.
...
Now, the registration data of the vehicles is added by a left_join.
...
The following filter shows that each affected vehicle was assigned to a registration.
