...
we noticed that the number in the column "werksnummer" is not matching the corresping number in the ID for 122. But since this is irrevelavnt for the further analysis, the dataset error was not corrected.

```{r, results='markup'}
Production_K7 <- fread(file="Data/Data/Logistikverzug/Komponente_K7.csv") %>%
  as_tibble() %>%
  mutate(production_date=as.Date(Produktionsdatum))
head(Production_K7)

Delivery_K7 <- fread(file="Data/Data/Logistikverzug/Logistics_delay_K7.csv") %>%
  as_tibble() %>%
  mutate(good_arrrival=as.Date(Wareneingang))
head(Delivery_K7)
```
...
```{r}
Production_K7 <-Production_K7[,c("IDNummer","production_date")]
head(Production_K7)

Delivery_K7 <-Delivery_K7[,c("IDNummer","good_arrrival")]
head(Delivery_K7)
```
...
Our first step in this exercise is to create a iterable list of the 
```{r, results='markup'}
#create list of file correspoding to the parts for the compoments
komponente_teil <- list.files("Data/Data/Komponente", full.names = T, pattern = "Bestandteile")
head(komponente_teil)
```
...
```{r, results='markup'}
#create function to find the compoments which use part 4
b <- bind_rows(lapply(komponente_teil,task_3_1))
head(b)
```
